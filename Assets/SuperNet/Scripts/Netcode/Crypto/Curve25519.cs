...
namespace SuperNet.Netcode.Crypto {
	...
	public static class Curve25519 {
		...
		/// <summary>Group order (a prime near 2^252+2^124).</summary>
		...
		/// <summary>Private key clamping (inline, for performance).</summary>
		/// <param name="key">[out] Random 32 bytes.</param>
		public static void ClampPrivateKeyInline(byte[] key) {
			...
			if (key == null) {
				...
				throw new ArgumentException(string.Format(
					"key must be 32 bytes long (but was {0} bytes long)", key.Length
				), nameof(key));
			}
		...
		/// <summary>Private key clamping.</summary>
		/// <param name="rawKey">[in] Random 32 bytes</param>
		/// <returns>Clamped private key.</returns>
		public static byte[] ClampPrivateKey(byte[] rawKey) {

			// Validate
			if (rawKey == null) {
				throw new ArgumentNullException(nameof(rawKey), "Raw key is null");
			} else if (rawKey.Length != 32) {
				throw new ArgumentException(string.Format(
					"rawKey must be 32 bytes long (but was {0} bytes long)", rawKey.Length
				), nameof(rawKey));
			}
		...
		/// <summary>Create a random clamped private key.</summary>
		/// <returns>Random 32 bytes that are clamped to a suitable private key.</returns>
		public static byte[] CreateRandomPrivateKey() {
			byte[] privateKey = new byte[32];
			using (RandomNumberGenerator random = RandomNumberGenerator.Create()) random.GetBytes(privateKey);
			ClampPrivateKeyInline(privateKey);
			return privateKey;
		}

		/// <summary>Generate key-pair (inline, for performance).</summary>
		...
		public static void KeyGenInline(byte[] publicKey, byte[] signingKey, byte[] privateKey) {
			...
			// Validate
			if (publicKey == null) {
				throw new ArgumentNullException(nameof(publicKey), "No public key");
			} else if (publicKey.Length != 32) {
				throw new ArgumentException(string.Format(
					"publicKey must be 32 bytes long (but was {0} bytes long)", publicKey.Length
				), nameof(publicKey));
			} else if (signingKey == null) {
				throw new ArgumentNullException(nameof(signingKey));
			} else if (signingKey.Length != 32) {
				throw new ArgumentException(string.Format(
					"signingKey must be 32 bytes long (but was {0} bytes long)", signingKey.Length
				), nameof(signingKey));
			} else if (privateKey == null) {
				throw new ArgumentNullException(nameof(privateKey));
			} else if (privateKey.Length != 32) {
				throw new ArgumentException(string.Format(
					"privateKey must be 32 bytes long (but was {0} bytes long)", privateKey.Length
				), nameof(privateKey));
			}
		...
		/// <summary>Generate the public key out of the clamped private key.</summary>
		/// <param name="privateKey">[in] Private key (must be clamped).</param>
		...
		/// <summary>Generate the public key out of the clamped private key (inline, for performance).</summary>
		/// <param name="privateKey">[in] Private key (must be clamped).</param>
		/// <param name="publicKey">[out] Public key.</param>
		...
		/// <summary>Generate signing key out of the clamped private key.</summary>
		/// <param name="privateKey">[in] Private key (must be clamped).</param>
		...
		private static void Core(byte[] publicKey, byte[] signingKey, byte[] privateKey, byte[] peerPublicKey) {
			...
			if (publicKey == null) {
				...
				throw new ArgumentException(string.Format(
					"signingKey must be null or 32 bytes long (but was {0} bytes long)", signingKey.Length
				), nameof(signingKey));
			} else if (privateKey == null) {
				throw new ArgumentNullException(nameof(privateKey));
			} else if (privateKey.Length != 32) {
				...
				throw new ArgumentException(string.Format(
					"peerPublicKey must be null or 32 bytes long (but was {0} bytes long)", peerPublicKey.Length
