...
namespace SuperNet.Examples.BPP {
	...
	public class BPP_Player : NetworkComponent {
		...
		private void Reset() {
			...
			NetworkPrefab = GetComponentInChildren<NetworkPrefab>();
		...
		private void Awake() {
			...
			Game = FindObjectOfType<BPP_Game>();
		}
		...
		public void SetAuthority(bool authority) {
			// Called by BPP_PlayerSpawner when a local player is spawned
			Authority = authority;
			if (authority) {
		...
		public void SetName(string name) {
			// Called by BPP_Game when name is updated
			if (Authority) {
		...
		public void SetColor(Color32 color) {
			// Called by BPP_Game when color is updated
			if (Authority) {
				// Update info, send network message and notify game ui
				Color = color;
		...
		public void SendChat(string message) {
			// Called by BPP_Game when a chat message is sent
			if (Authority) {
				// Send to everybody
